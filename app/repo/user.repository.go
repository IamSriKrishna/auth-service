...
func (r *userRepository) GetDashboardStats(
	customerType *string,
	fromDate *time.Time,
	toDate *time.Time,
	...
	if err != nil {
		fmt.Printf(
			"failed to fetch membership count: %v\n",
			err,
		)
		...
		stats["membership_users"] =
			membershipData.Count

		stats["membership_last_updated_at"] =
			membershipData.LastUpdatedAt
	}
...
func (r *userRepository) GetByPhone(
	phone string,
) (*models.User, error) {
	var user models.User
...
func (r *userRepository) GetByGoogleID(
	googleID string,
) (*models.User, error) {
	var user models.User
...
func (r *userRepository) GetByAppleID(
	appleID string,
) (*models.User, error) {
	var user models.User
...
func (r *userRepository) ListWithFilters(
	...
	if search != "" {
		...
		query = query.Where(
			"email LIKE ? OR phone LIKE ? OR username LIKE ?",
			searchTerm,
			searchTerm,
			searchTerm,
		)
...
func (r *userRepository) UpdatePasswordChangedAt(
	id uint,
) error {
	now := time.Now()
