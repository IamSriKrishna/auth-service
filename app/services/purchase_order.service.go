...
type PurchaseOrderService interface {
	...
	CreatePurchaseOrder(
		poInput *input.CreatePurchaseOrderInput,
		userID string,
	) (*output.PurchaseOrderOutput, error)
	...
	GetAllPurchaseOrders(
		limit int,
		offset int,
	) (*output.PurchaseOrderListOutput, error)

	UpdatePurchaseOrder(
		id string,
		poInput *input.UpdatePurchaseOrderInput,
		userID string,
	) (*output.PurchaseOrderOutput, error)
	...
	ReorderProductGroup(
		reorderInput *input.ReorderProductGroupInput,
		userID string,
	) (*output.PurchaseOrderOutput, error)

	// Company-scoped methods used by authenticated routes.
	CreatePurchaseOrderForCompany(
		poInput *input.CreatePurchaseOrderInput,
		userID string,
		companyID uint,
	) (*output.PurchaseOrderOutput, error)

	GetPurchaseOrderByCompany(
		id string,
		companyID uint,
	) (*output.PurchaseOrderOutput, error)

	GetAllPurchaseOrdersByCompany(
		companyID uint,
		limit int,
		offset int,
	) (*output.PurchaseOrderListOutput, error)

	UpdatePurchaseOrderForCompany(
		id string,
		poInput *input.UpdatePurchaseOrderInput,
		userID string,
		companyID uint,
	...
	DeletePurchaseOrderForCompany(
		id string,
		companyID uint,
	) error

	GetPurchaseOrdersByVendorAndCompany(
		vendorID uint,
		companyID uint,
		limit int,
		offset int,
	...
	GetPurchaseOrdersByCustomerAndCompany(
		customerID uint,
		companyID uint,
		limit int,
		offset int,
	...
	GetPurchaseOrdersByStatusAndCompany(
		status string,
		companyID uint,
		limit int,
		offset int,
	...
	UpdatePurchaseOrderStatusForCompany(
		id string,
		status domain.PurchaseOrderStatus,
		userID string,
		companyID uint,
...
func (s *purchaseOrderService) validatePurchaseOrderUserCompany(
	userID string,
	companyID uint,
) error {
	if companyID == 0 {
...
func (s *purchaseOrderService) validatePurchaseOrderInputCompany(
	poInput *input.CreatePurchaseOrderInput,
	companyID uint,
) error {
	if poInput == nil {
	...
	for _, lineItem := range poInput.LineItems {
...
func (s *purchaseOrderService) validatePurchaseOrderUpdateCompany(
	poInput *input.UpdatePurchaseOrderInput,
	companyID uint,
) error {
	if poInput == nil {
...
func (s *purchaseOrderService) CreatePurchaseOrderForCompany(
	poInput *input.CreatePurchaseOrderInput,
	userID string,
	companyID uint,
) (*output.PurchaseOrderOutput, error) {
	if err := s.validatePurchaseOrderInputCompany(
		poInput,
		companyID,
	); err != nil {
		return nil, err
	...
	// Existing method is retained and performs the complete PO creation.
	// Validation above ensures every referenced record belongs to companyID.
	return s.CreatePurchaseOrder(poInput, userID)
}

func (s *purchaseOrderService) GetPurchaseOrderByCompany(
	id string,
	companyID uint,
) (*output.PurchaseOrderOutput, error) {
	purchaseOrder, err := s.poRepo.FindByIDAndCompany(
		id,
		companyID,
	)
	if err != nil {
	...
	return output.ToPurchaseOrderOutput(purchaseOrder)
}

func (s *purchaseOrderService) GetAllPurchaseOrdersByCompany(
	companyID uint,
	limit int,
	offset int,
) (*output.PurchaseOrderListOutput, error) {
	purchaseOrders, total, err :=
		s.poRepo.FindAllByCompany(
			companyID,
			limit,
			offset,
		)
	...
	return purchaseOrdersToListOutput(
		purchaseOrders,
		total,
	)
}

func (s *purchaseOrderService) UpdatePurchaseOrderForCompany(
	id string,
	poInput *input.UpdatePurchaseOrderInput,
	userID string,
	companyID uint,
	...
	if err := s.validatePurchaseOrderUpdateCompany(
		poInput,
		companyID,
	); err != nil {
		return nil, err
	...
	// The existing update logic is preserved.
	return s.UpdatePurchaseOrder(
		id,
		poInput,
		userID,
	)
...
func (s *purchaseOrderService) DeletePurchaseOrderForCompany(
	id string,
	companyID uint,
) error {
	if _, err := s.poRepo.FindByIDAndCompany(
	...
	// Existing method retains stock reversal and ledger cleanup.
	return s.DeletePurchaseOrder(id)
}

func (s *purchaseOrderService) GetPurchaseOrdersByVendorAndCompany(
	vendorID uint,
	companyID uint,
	limit int,
	offset int,
	...
	purchaseOrders, total, err :=
		s.poRepo.FindByVendorAndCompany(
			vendorID,
			companyID,
			limit,
			offset,
...
func (s *purchaseOrderService) GetPurchaseOrdersByCustomerAndCompany(
	customerID uint,
	companyID uint,
	limit int,
	offset int,
	...
	purchaseOrders, total, err :=
		s.poRepo.FindByCustomerAndCompany(
			customerID,
			companyID,
			limit,
			offset,
...
func (s *purchaseOrderService) GetPurchaseOrdersByStatusAndCompany(
	status string,
	companyID uint,
	limit int,
	offset int,
	...
	purchaseOrders, total, err :=
		s.poRepo.FindByStatusAndCompany(
			status,
			companyID,
			limit,
			offset,
...
func (s *purchaseOrderService) UpdatePurchaseOrderStatusForCompany(
	id string,
	status domain.PurchaseOrderStatus,
	userID string,
	companyID uint,
	...
	// Existing method retains inventory and variant stock synchronization.
	return s.UpdatePurchaseOrderStatus(
		id,
		status,
		userID,
	)
...
func (s *purchaseOrderService) ReorderProductGroupForCompany(
	reorderInput *input.ReorderProductGroupInput,
	userID string,
	companyID uint,
) (*output.PurchaseOrderOutput, error) {
	...
	if err := s.validatePurchaseOrderUserCompany(
		userID,
		companyID,
	); err != nil {
		return nil, err
	...
	for _, component := range reorderInput.Components {
		if _, err := s.productRepo.FindByIDAndCompany(
			component.ProductID,
			companyID,
		); err != nil {
	...
	createdPurchaseOrder, err := s.ReorderProductGroup(
		reorderInput,
		userID,
	)
	if err != nil {
	...
	return s.GetPurchaseOrderByCompany(
		createdPurchaseOrder.ID,
		companyID,
	)
}

func purchaseOrdersToListOutput(
	...
	for index, purchaseOrder := range purchaseOrders {
		purchaseOrderOutput, err :=
			output.ToPurchaseOrderOutput(&purchaseOrder)
		if err != nil {
			return nil, err
		}

		outputs[index] = *purchaseOrderOutput
	}
